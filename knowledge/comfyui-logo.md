...
- 그림 글자(붓글씨)는 **img2img 로 간다**: ControlNet canny 만으로는 SDXL 이 도형·바탕을 뒤집어 칸을 통째로 빨갛게 칠한다(시도 네 번 다 실패).
...
- 파이프라인: `work/comfy_run.py`(프롬프트 파일에 `NEG:` 줄을 두면 부정 프롬프트) → 상자 잘라 1배 축소·2값화 → 팔레트 색칠 → 이식소 `tools/gfx_insert.py build`(`--merge`) → 화면 캡처.
- 도구: `~/ss2/work_lang/v10/gfx/gfx_grid.py` (plan/quant/build/show) — 자산 JSON 의 «그릴 수 있는 칸»을 그림으로 보여 준다. 삽입 자체는 이식소 도구를 쓴다.
- **한패 자산의 벽**: `h_shobu` 는 지금 한글 글자가 잉크를 놓은 칸에만 타일이 있다(그릴 수 있는 칸 40/108, 모양도 들쭉날쭉). 순정 `p_shobu` 는 66칸 직사각형이라 새 글자가 그대로 들어간다.
  → 새 붓글씨를 한패에 넣으려면 **한패 쪽 타일 배정을 순정처럼 넓혀야** 한다(평면1 지도 손질 = 이식소 몫). 이번 시범은 순정 롬에 넣어 화면으로 확인했다: `~/ss2/work_lang/v10/gfx/shobu_ingame.png`.
- 화면 대조는 이식소 경고대로 «각 롬을 새 폴더에 복사 + .flash 지우고» 돌렸고, 라운드 시작 배너(입력 뒤 첫 프레임)에서 찍었다.
